...
var input_password = RegisterLoginComponent.initInputPassWord({defaultPlaceHolder:'新密码',focusPlaceHolder:'请输入6-12位字母、数字或符号'});
...
$('#pwdmoile-btn').click(function(){
    ...
    if (!(input_mobile.status && input_mobileActivateCode.status && input_password.status && input_imageVerifyCode.status)){
        return false;
    }
...
$('#get-code-btn').live('click', function(){
    ...
    $.when((function(){
        ...
        if (input_mobile.status == false || input_imageVerifyCode.status == false) return false;
