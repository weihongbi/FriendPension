...
function Captcha(config) {
 ...
 // JavaScript Document
$(function(){
	$(".left_sidebar h4").click(function(){
			$(this).addClass("pink_cur").siblings().removeClass("pink_cur") ;

            $(this).next(".sidebar_list").children().first().addClass("bg_gray").siblings().removeClass("bg_gray");
            var valuedivid = $(this).next(".sidebar_list").attr('valuedivid');
            $("#"+valuedivid+" .right_tab").hide().eq(0).show();
            
            $(this).next(".sidebar_list").slideDown(500).css('display','block').siblings(".sidebar_list").slideUp(500);
			$(".right_con .switch").hide().eq($(this).index()-$(this).index()/2).show();
	})
	$("#guide_list1").click(function(){
            $(this).next(".sidebar_list").css('display','none');
			$("#guide_tab1 .right_tab").hide().eq($(this).index()).show();
	})
	$("#guide_list2 li").click(function(){
			$(this).addClass("bg_gray").siblings().removeClass("bg_gray");
			$("#guide_tab2 .right_tab").hide().eq($(this).index()).show();
	})
	$("#guide_list3 li").click(function(){
			$(this).addClass("bg_gray").siblings().removeClass("bg_gray");
			$("#guide_tab3 .right_tab").hide().eq($(this).index()).show();
	})
	$("#guide_list4 li").click(function(){
			$(this).addClass("bg_gray").siblings().removeClass("bg_gray");
			$("#guide_tab4 .right_tab").hide().eq($(this).index()).show();
	})
	$(".safe_ul li").click(function(){
			$(this).addClass("safe_li_bor").siblings().removeClass("safe_li_bor");
			$(".safe_con").hide().eq($(this).index()).show();
	})
	$(".safe_ul li").mouseover(function(){
			$(this).addClass("safe_li_bor").siblings().removeClass("safe_li_bor");
			$(".safe_con").hide().eq($(this).index()).show();
	});
}) 
